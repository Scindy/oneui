...
const { primary, success, danger, warning, info, primaryRGB, successRGB, dangerRGB, warningRGB, infoRGB } = Color;
const { ff, ffRGB, fe, feRGB, fc, fcRGB, f9, f9RGB, f6, f6RGB, f3, f3RGB, f0, f0RGB, bg, bgRGB, bp, bpRGB, bs, bsRGB, bd, bdRGB, bw, bwRGB, bi, biRGB } = Color;
const { primaryG, successG, dangerG, warningG, infoG } = Color;
const { ffG, feG, fcG, f9G, f6G, f3G, f0G, bgG, bpG, bsG, bdG, bwG, biG } = Color;
/*
 * 媒体查询
...
const getColor = (c, o, over) => {
  switch (c) {
    case 'p':
      c = o || o === 0 ? `rgba(${primaryRGB},${'.' + o})` : over ? primaryG : primary;
      break;
    case 's':
      c = o || o === 0 ? `rgba(${successRGB},${'.' + o})` : over ? successG : success;
      break;
    case 'd':
      c = o || o === 0 ? `rgba(${dangerRGB},${'.' + o})` : over ? dangerG : danger;
      break;
    case 'w':
      c = o || o === 0 ? `rgba(${warningRGB},${'.' + o})` : over ? warningG : warning;
      break;
    case 'i':
      c = o || o === 0 ? `rgba(${infoRGB},${'.' + o})` : over ? infoG : info;
      break;
    case 'ff':
      c = o || o === 0 ? `rgba(${ffRGB},${'.' + o})` : over ? ffG : ff;
      break;
    case 'fe':
      c = o || o === 0 ? `rgba(${feRGB},${'.' + o})` : over ? feG : fe;
      break;
    case 'fc':
      c = o || o === 0 ? `rgba(${fcRGB},${'.' + o})` : over ? fcG : fc;
      break;
    case 'f9':
      c = o || o === 0 ? `rgba(${f9RGB},${'.' + o})` : over ? f9G : f9;
      break;
    case 'f6':
      c = o || o === 0 ? `rgba(${f6RGB},${'.' + o})` : over ? f6G : f6;
      break;
    case 'f3':
      c = o || o === 0 ? `rgba(${f3RGB},${'.' + o})` : over ? f3G : f3;
      break;
    case 'f0':
      c = o || o === 0 ? `rgba(${f0RGB},${`.${o}`})` : over ? f0G : f0;
      break;
    case 'bg':
      c = o || o === 0 ? `rgba(${bgRGB},${`.${o}`})` : over ? bgG : bg;
      break;
    case 'bp':
      c = o || o === 0 ? `rgba(${bpRGB},${`.${o}`})` : over ? bpG : bp;
      break;
    case 'bs':
      c = o || o === 0 ? `rgba(${bsRGB},${`.${o}`})` : over ? bsG : bs;
      break;
    case 'bd':
      c = o || o === 0 ? `rgba(${bdRGB},${`.${o}`})` : over ? bdG : bd;
      break;
    case 'bw':
      c = o || o === 0 ? `rgba(${bwRGB},${`.${o}`})` : over ? bwG : bw;
      break;
    case 'bi':
      c = o || o === 0 ? `rgba(${biRGB},${`.${o}`})` : over ? biG : bi;
      break;
    default:
      c = o || o === 0 ? `rgba(${ffRGB},${`.${o}`})` : '';
      break;
  }
  return c;
};
...
export const Button = styled.button`
  ...
  `
    ...
    ${props => props['h'] && typeof props['h'] === 'number' && `height:${props['h']}px;`}
    ${props => props['br'] && typeof props['br'] === 'number' && `border-radius:${props['br']}px;`}
    ${props => props['o'] && typeof props['o'] === 'number' && props['o'] === 0 ? 'opacity:0' : `opacity:.${props['o']};`}
