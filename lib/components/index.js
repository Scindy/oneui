...
const position = props => {
	...
	const { _ppr, _ppa, _ppf, _ppi, _pz, __pt, __pr, __pb, __pl, _pr, _pa, _pf, _pi, _z, _t, _r, _b, _l, nwp } = props;
	let combination = '';
	if (_dpr || _dpa || _dpf || _dpi || _dz || _dt || _dr || _db || _dl) {
		let css = '';
	...
	if (_tpr || _tpa || _tpf || _tpi || _tz || _tt || _tr || _tb || _tl) {
		let css = '';
	...
	if (_ppr || _ppa || _ppf || _ppi || _pz || _pt || _pr || _pb || _pl) {
		let css = '';
	...
	if (_pr || _pa || _pf || _pi || _z || __t || __r || __b || __l) {
		let css = '';
		...
		if ((__t && typeof __t === 'number') || __t === 0) {
			css += `top:${__t}px;`;
		}
		if ((__r && typeof __r === 'number') || __r === 0) {
			css += `top:${__r}px;`;
		}
		if ((__b && typeof __b === 'number') || __b === 0) {
			css += `top:${__b}px;`;
		}
		if ((__l && typeof __l === 'number') || __l === 0) {
			css += `top:${__l}px;`;
		}
		combination += css;
	}
